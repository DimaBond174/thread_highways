	...
	std::int32_t life_time)
{
	...
	auto subscription = channel->subscribe(
		[scope = std::move(scope), protector = self_protector, life_time, name = life_time](
			std::string publication) mutable
		{
			scope->print(std::to_string(name)
							 .append(" life_time subscriber received: ")
...
void connections_notifier_simple()
{
	...
	{
		auto subscriber1 = publisher->subscribe_channel()->subscribe(
...
void connections_notifier_start_stop_microservice()
{
	...
	private:
		const hi::HighWayProxyPtr highway_;
