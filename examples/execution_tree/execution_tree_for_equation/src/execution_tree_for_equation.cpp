...
void test_1()
{
	...
	//	auto sum_x_node =
	//		hi::make_self_shared<SumCalculator<300>>(highway_mailbox, execution_tree.current_executed_node_publisher(),
	//1); 	auto sum_y_node = 		hi::make_self_shared<SumCalculator<200>>(highway_mailbox,
	//execution_tree.current_executed_node_publisher(), 2); 	auto sum_z_node =
	//		hi::make_self_shared<SumCalculator<100>>(highway_mailbox, execution_tree.current_executed_node_publisher(),
	//3);
	...
	//	auto subtraction_logic_node = hi::make_self_shared<hi::OperationWithTwoOperandsFutureNode<double, double,
	//double>>( 		hi::OperationWithTwoOperandsFutureNodeLogic<double, double, double>::create(
	//			[](double operand1, double operand2, hi::IPublisher<double> & result_publisher)
	//			{
	//				result_publisher.publish(operand1 - operand2);
	//			},
	...
	//		auto process_result_future_node = hi::FutureNode<double, double>::create(
	//				 [&](double result, hi::IPublisher<double> & next, hi::INode &node, const std::atomic<std::uint32_t> &
	//global_run_id, 			   const std::uint32_t your_run_id)
	//				{
	//				for (uint16_t progress = 1;
	//					 progress < 100 && your_run_id == global_run_id.load(std::memory_order_acquire);
	//					 ++progress)
	//				{
	//					node.publish_progress_state(true, progress);
	//				}
	//				node.publish_progress_state(false, 100);
	//				next.publish(result);
	//				}, highway->protector(), highway, __FILE__, __LINE__, execution_tree.current_executed_node_publisher(),
	//6
	//		);
	...
	//			[&](hi::CurrentExecutedNode publication, const std::atomic<std::uint32_t> &, const std::uint32_t)
	//mutable
	//			{
