...
void aggregate_geo_position()
{
	...
	{
	public:
		void operator()(GeoPosition publication, const std::int32_t label, hi::Publisher<GeoPosition> & publisher)
		{
			operands_map_[label] = publication;
			...
			publisher.publish_direct(GeoPosition{sum_latitude / size, sum_longitude / size});
		}
	...
	auto aggregating_future =
		hi::make_self_shared<hi::DefaultNode<GeoPosition, GeoPosition, AggregatingBundle>>(highway, 0);

	auto subscription = hi::create_subscription_for_new_only<GeoPosition>(
		[&](GeoPosition result)
		{
			scope.print(std::string{"GeoPosition ["}
							.append(std::to_string(result.latitude_))
							.append(", ")
							.append(std::to_string(result.longitude_))
							.append("]"));
		});

	aggregating_future->subscribe_direct(subscription);
	...
	const auto location_sensor = [&](std::int32_t label, const double d_latitude, const double d_longitude)
	{
		...
		sensor->subscribe(aggregating_future->in_param_highway_channel(label));
