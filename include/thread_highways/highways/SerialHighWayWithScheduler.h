...
private:
	...
	void main_worker_thread_loop(const std::uint32_t your_run_id)
	{
		...
		const auto execute_reschedulable_runnable = [&](Holder<ReschedulableRunnable> * holder)
		{
			...
			{
				bundle_.log(
					std::string{"OtherLogic:exception:"}.append(e.what()),
					holder->t_.get_code_filename(),
					holder->t_.get_code_line());
			}
			catch (...)
			{
				bundle_.log("OtherLogic:exception:", holder->t_.get_code_filename(), holder->t_.get_code_line());
			}
			const auto after_time = std::chrono::steady_clock::now();
			const auto time_diff = std::chrono::duration_cast<std::chrono::milliseconds>(after_time - before_time);
			if (bundle_.error_logger_ && time_diff > max_task_execution_time)
			{
				bundle_.log(":OtherLogic:stuck", holder->t_.get_code_filename(), holder->t_.get_code_line());
			}
		...
		{
			bundle_.what_is_running_now_.store(HighWayBundle::WhatRunningNow::OtherLogic, std::memory_order_relaxed);
			...
			else
			{
				...
				bundle_.mail_box_.wait_for_new_messages(timer_executes_each_ns_);
			}
