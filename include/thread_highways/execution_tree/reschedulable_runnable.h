...
public:
	 ...
	 */
	template <typename R>
	static ReschedulableRunnable create(
		R && r,
		...
		unsigned int line)
	{
		struct RunnableHolderImpl : public RunnableHolder
		{
			 ...
			 */
			void operator()(Schedule & schedule, [[maybe_unused]] const std::atomic<bool> & keep_execution) override
			{
				if constexpr (std::is_invocable_v<R, Schedule &, const std::atomic<bool> &>)
				{
...
private:
	struct RunnableHolder
	{
		...
		virtual void operator()(Schedule & schedule, const std::atomic<bool> & keep_execution) = 0;
	};

	ReschedulableRunnable(
		RunnableHolder * runnable,
		std::chrono::steady_clock::time_point next_execution_time,
		const char * filename,
		unsigned int line)
