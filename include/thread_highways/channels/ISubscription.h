...
inline bool send_with_params(R& callback, [[maybe_unused]] Publication publication, [[maybe_unused]] const std::atomic<bool>& keep_execution)
{
...
inline std::shared_ptr<ISubscription<Publication>> create_subscription(R && callback)
{
    ...
    {
        Subscription(R && callback)  : callback_{std::move(callback)} { }
        ...
        )
{
    if (send_may_fail)
    {
        struct Subscription : public ISubscription<Publication>
        {
            Subscription(std::weak_ptr<Subscription> self, R && callback, HighWayWeakPtr highway, const char* filename, unsigned int line)
                : self_{std::move(self)}
            ...
            bool send(Publication publication) final
            {
                auto runnable = Runnable::create(
                    [&, alive = self_, p = std::move(publication)](const std::atomic<bool>& keep_execution) mutable
                    {
                        if (auto lock = alive.lock())
                        {
                            if (!send_with_params<R, Publication>(callback_, std::move(p), keep_execution))
                            {
                                throw Exception("Error: subscription callback must take parameters: Publication publication, [[maybe_unused]] const std::atomic<bool>& keep_execution", filename_, line_);
                            }
                        }
                    }, filename_, line_
                );
...
{
    return create_subscription<Publication, R>(std::move(callback), make_weak_ptr(highway), filename, line, send_may_fail);
}
...
{
    return create_subscription_for_new_only<Publication, R>(std::move(callback), make_weak_ptr(highway), filename, line, send_may_fail);
}
...
struct ISubscribeHere
{
    ...
    {
        auto subscription = for_new_only
            ? create_subscription_for_new_only<Publication, R>(std::move(callback))
    ...
    template<typename R>
    std::shared_ptr<ISubscription<Publication>> subscribe(R&& callback, HighWayWeakPtr highway,
                                                          const char* filename, unsigned int line,
                                                          bool send_may_fail = true,
                                                          bool for_new_only = false)
    {
        auto subscription = for_new_only
            ? create_subscription_for_new_only<Publication, R>(std::move(callback), std::move(highway), filename, line, send_may_fail)
            : create_subscription<Publication, R>(std::move(callback), std::move(highway), filename, line, send_may_fail);
        subscribe(subscription);
        return subscription;
    }

    template<typename R>
    std::shared_ptr<ISubscription<Publication>> subscribe(R&& callback, std::shared_ptr<HighWay> highway,
                                                          const char* filename, unsigned int line,
                                                          bool send_may_fail = true,
                                                          bool for_new_only = false)
    {
        auto highway_weak = std::make_shared<hi::HighWayWeak>(std::move(highway));
        auto subscription = for_new_only
            ? create_subscription_for_new_only<Publication, R>(std::move(callback), std::move(highway_weak), filename, line, send_may_fail)
            : create_subscription<Publication, R>(std::move(callback), std::move(highway_weak), filename, line, send_may_fail);
        subscribe(subscription);
        return subscription;
    }
};
